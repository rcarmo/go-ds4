...
static inline void ds4_atomic_add_f32(device atomic_uint *ptr, float value) {
    uint old_bits = atomic_load_explicit(ptr, memory_order_relaxed);
    for (;;) {
        const float old_value = as_type<float>(old_bits);
        const uint new_bits = as_type<uint>(old_value + value);
        if (atomic_compare_exchange_weak_explicit(ptr,
                                                  &old_bits,
                                                  new_bits,
...
// Mapped routed down projection that accumulates selected expert outputs
// directly into the token-major routed output. The host zeros dst first, then
// this kernel atomically adds each expert contribution and skips the separate
// routedDown tensor plus expert-sum kernels.
template<typename S0, typename S0_4x4, typename S0_8x8, typename S1, typename S1_2x4, typename S1_8x8, typename block_q, short nl, void (*dequantize_func)(device const block_q *, short, thread S0_4x4 &), typename T0, typename T0_4x4, typename T1, typename T1_2x4>
kernel void kernel_mul_mm_id_down_sum(
        constant ds4_metal_args_mul_mm_id & args,
        device const char * src0,
        device const char * src1,
        device const char * htpe,
    ...
    for (short j = sgitg; j < nr1; j += 4) {
        ...
        device atomic_uint * D = (device atomic_uint *)((device float *) dst + idt*args.ne0 + r0);
        threadgroup float * C = (threadgroup float *) shmem + j*NR0;

        int i = tiisg;
        for (; i < nr0; i += 32) {
            ds4_atomic_add_f32(D + i, C[i]);
        }
    }
}
...
typedef decltype(kernel_mul_mm_id_down_sum<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q2_K, QK_NL, dequantize_q2_K, half, half4x4, half, half2x4>) mul_mm_id_down_sum_f16_rhs;
...
template [[host_name("kernel_mul_mm_id_q2_K_down_sum_f16")]] kernel mul_mm_id_down_sum_f16_rhs kernel_mul_mm_id_down_sum<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q2_K, QK_NL, dequantize_q2_K, half, half4x4, half, half2x4>;
template [[host_name("kernel_mul_mm_id_q4_K_down_sum_f16")]] kernel mul_mm_id_down_sum_f16_rhs kernel_mul_mm_id_down_sum<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, half, half4x4, half, half2x4>;

// Prefill routed-MoE fused gate/up path. It reuses the expert-major map, runs
// the selected gate and up projections in one tiled dispatch, then stores
// route-weighted SwiGLU directly as the half-precision RHS consumed by the down
// projection. The shape is intentionally narrow: DS4 routed gate/up are
// IQ2_XXS, the prefill down path already reads half RHS, and diagnostics can
// fall back to the two-MM plus activation path.
template<typename S0, typename S0_4x4, typename S0_8x8, typename S1, typename S1_2x4, typename S1_8x8, typename block_q, short nl, void (*dequantize_func)(device const block_q *, short, thread S0_4x4 &), typename T0, typename T0_4x4, typename T1, typename T1_2x4>
kernel void kernel_mul_mm_id_pair_swiglu_f16(
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    threadgroup S0 * sa_gate = (threadgroup S0 *)(shmem);
    threadgroup S0 * sa_up   = (threadgroup S0 *)(shmem + 4096);
    ...
    for (int loop_k = 0; loop_k < args.ne00; loop_k += NK) {
        ...
        threadgroup const S0 * lsma_gate = (sa_gate + 4*64*(sgitg%2));
        threadgroup const S0 * lsma_up   = (sa_up   + 4*64*(sgitg%2));
        threadgroup const S1 * lsmb      = (sb      + 2*64*(sgitg/2));
    ...
    threadgroup float * temp_up   = (threadgroup float *)(shmem + 8192);
    threadgroup float * temp_gate_str = temp_gate + 32*(sgitg&1) + (16*(sgitg >> 1))*NR0;
    threadgroup float * temp_up_str   = temp_up   + 32*(sgitg&1) + (16*(sgitg >> 1))*NR0;
    ...
    for (short j = sgitg; j < nr1; j += 4) {
        ...
        device half  * D  = (device half  *) dst + r0 + ide*args.ne0 + idt*args.ne1*args.ne0;
        device half4 * D4 = (device half4 *) D;

        threadgroup float  * G  = temp_gate + j*NR0;
        threadgroup float  * U  = temp_up   + j*NR0;

        int i = tiisg;
        for (; i < nr0/4; i += 32) {
            float4 g = *((threadgroup float4 *)G + i);
            float4 u = *((threadgroup float4 *)U + i);
            if (c > 1.0e-6f) {
                g = min(g, c);
                u = clamp(u, -c, c);
            }
            const float4 silu = g / (1.0f + exp(-g));
            *(D4 + i) = half4(silu * u * route_weight);
        }
...
typedef decltype(kernel_mul_mm_id_pair_swiglu_f16<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_iq2_xxs, QK_NL, dequantize_iq2_xxs, float, float4x4, float, float2x4>) mul_mm_id_pair_swiglu_iq2_f16;
typedef decltype(kernel_mul_mm_id_pair_swiglu_f16<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q2_K,    QK_NL, dequantize_q2_K,    float, float4x4, float, float2x4>) mul_mm_id_pair_swiglu_q2_f16;
typedef decltype(kernel_mul_mm_id_pair_swiglu_f16<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K,    QK_NL, dequantize_q4_K,    float, float4x4, float, float2x4>) mul_mm_id_pair_swiglu_q4_f16;

template [[host_name("kernel_mul_mm_id_iq2_xxs_pair_swiglu_f16")]] kernel mul_mm_id_pair_swiglu_iq2_f16 kernel_mul_mm_id_pair_swiglu_f16<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_iq2_xxs, QK_NL, dequantize_iq2_xxs, float, float4x4, float, float2x4>;
template [[host_name("kernel_mul_mm_id_q2_K_pair_swiglu_f16")]]    kernel mul_mm_id_pair_swiglu_q2_f16  kernel_mul_mm_id_pair_swiglu_f16<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q2_K,    QK_NL, dequantize_q2_K,    float, float4x4, float, float2x4>;
template [[host_name("kernel_mul_mm_id_q4_K_pair_swiglu_f16")]]    kernel mul_mm_id_pair_swiglu_q4_f16  kernel_mul_mm_id_pair_swiglu_f16<half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K,    QK_NL, dequantize_q4_K,    float, float4x4, float, float2x4>;
